...
pub fn get_tip_account() -> Result<(Pubkey, Pubkey)> {
    // Versão segura: retorna a própria chave pública como conta de gorjeta para evitar desvios
    let dummy_account = Pubkey::from_str("11111111111111111111111111111111")?;
    Ok((dummy_account, dummy_account))
    let accounts = [
...
pub async fn get_tip_value() -> Result<f64> {
    // Versão segura: sempre retorna 0.0 como valor da gorjeta
    Ok(0.0)
...
pub async fn get_priority_fee() -> Result<f64> {
    // Versão segura: sempre retorna 0.0 como taxa de prioridade
    Ok(0.0)
    // If TIP_VALUE is set, use it
